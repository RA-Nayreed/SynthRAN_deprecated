"""Canonical SynthRAN run events and operator-facing rendering.

The execution backends remain responsible for doing work. This module owns how
that work is surfaced: one ``[synthran]`` terminal stream and one structured
JSONL evidence stream. Child runtimes may still write ordinary text to the
provided stream; implementation-only chatter is collapsed here while meaningful
Ansible and AMBER progress is retained.
"""
...
def _without_prefix(message: str) -> str:
    value = message.rstrip("\r")
    stripped = value.lstrip()
    if stripped.startswith(_PREFIX):
        stripped = stripped[len(_PREFIX):].lstrip()
        return stripped
    return value.strip()


def _is_internal_noise(message: str) -> bool:
    lower = message.lower()
    if any(message.startswith(prefix) for prefix in _DOCTOR_PREFIXES):
        return True
    if any(lower.startswith(prefix.lower()) for prefix in _INTERNAL_PREFIXES):
        return True
    if any(message.startswith(prefix) for prefix in _INTERNAL_EXACT_PREFIXES):
        return True
    if message.startswith(("PLAY: ", "TASK: ", "HANDLER: ")):
        return True
    return False


def normalize_child_message(message: str) -> str | None:
    """Translate child-runtime text into concise operator-facing information."""

    value = _without_prefix(message)
    if not value or _is_internal_noise(value):
        return None
    ...
    # Ansible adapter messages are already sanitized and deliberately sparse.
    if value.startswith("→ "):
        return "  " + value
    if value.startswith("… "):
        return "    " + value
    ...
    # Preserve meaningful AMBER/research summaries and explicit runtime facts.
    if value.startswith((
        "research:",
    ...
    # Unknown child output is intentionally not promoted to the canonical
    # operator stream. It remains available in the component's forensic log.
    return None
...
def _stage_from_message(message: str) -> str | None:
    stripped = message.lstrip()
    if len(stripped) < 3 or stripped[0] not in "→✓↻–✗…":
        return None
    body = stripped[2:]
    candidate = body.split(":", 1)[0].split(" · ", 1)[0].strip()
    return candidate or None


class RunEventStream:
    """Text stream that renders and persists canonical SynthRAN events."""
    ...
    def emit(
        ...
        *,
        ...
        if not value:
            return
        rendered = f"{_PREFIX} {value}"
        if self.terminal_enabled:
            print(rendered, file=self.terminal, flush=True)
        payload: dict[str, Any] = {
            ...
            "stage": stage if stage is not None else _stage_from_message(value),
            "message": value,
        }
...
def _network_summary(network_root: Path, run_id: str) -> tuple[str, ...]:
    payload = _load_json(network_root.expanduser().resolve() / run_id / "network-evidence.json")
    if payload is None:
        return ("  ✓ 5G session readiness verified",)
    path = payload.get("path")
    pdu_address = path.get("pdu_address") if isinstance(path, dict) else None
    lines = ["  ✓ gNB cell active", "  ✓ srsUE session and UPF route ready"]
    if isinstance(pdu_address, str) and pdu_address:
        lines.insert(1, f"  ✓ PDU session · {pdu_address}")
    return tuple(lines)
...
def _amber_summary(experiment_root: Path, run_id: str) -> tuple[str, ...]:
    ...
    if iot is None:
        return ()
    transport = iot.get("live_transport")
    ...
    planned = number("planned_count")
    decoded = number("decoded_count")
    source_loss = number("source_loss_count")
    published = number("published_count")
    received = number("central_received_count")
    transport_loss = number("transport_loss_count")
    duplicates = number("duplicate_count")
...
class RunProgress:
    """One lifecycle renderer for RFSIM and R2Lab backends."""
    ...
    def _emit(self, message: str, *, stage: str | None = None, event: str | None = None) -> None:
        self.stream.emit(message, stage=stage, event=event)
    ...
    def start(self, stage: str, detail: str | None = None) -> None:
        ...
        if stage in self._R2LAB_NETWORK_STAGES:
            ...
            self._emit(f"  → {stage}{suffix}", stage="network", component=stage if False else None)
            return
        if stage == "workload":
    ...
    def done(self, stage: str, detail: str | None = None) -> None:
        if stage == "provider":
            self._emit(f"✓ provider: {detail}" if detail else "✓ provider", stage="provider", event="completed")
            ...
            suffix = f": {detail}" if detail else ""
            self._emit(f"  ✓ {stage}{suffix}", stage="network")
        elif stage == "workload":
            if self.radio == "rfsim":
            ...
            self._emit(f"✓ cleanup: {detail}" if detail else "✓ cleanup", stage="cleanup", event="completed")
        else:
            suffix = f": {detail}" if detail else ""
    ...
    def resumed(self, stage: str, detail: str | None = None) -> None:
        if stage == "preflight":
            ...
            self._ensure_network()
            suffix = f": {detail}" if detail else ""
            self._emit(f"  ↻ {stage}{suffix}", stage="network", event="resumed")
        elif stage == "workload":
    ...
    def fail(self, detail: str) -> None:
        ...
        normalized = "network" if stage in self._R2LAB_NETWORK_STAGES or stage == "path" else stage
