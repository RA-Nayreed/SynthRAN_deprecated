"""Experiment-side inventory helpers for 5g-Ansible deployments.

Deployment execution lives in :mod:`synthran.adapters.fiveg`.  This module only
parses generated inventories for experiment access and provides a read-only
doctor facade.  It deliberately does not certify which 5G topologies are
allowed; 5g-Ansible is the capability authority.
"""
...
class FiveGAnsibleError(FiveGAdapterError):
    """Compatibility error type for experiment-side 5g-Ansible helpers."""
...
def parse_inventory(text: str, *, source: Path) -> NetworkInventory:
    """Parse generated inventory facts without imposing a topology matrix."""
...
def validate_fiveg_checkout(lock, dependency_root: Path) -> Path:
    """Compatibility alias for the new thin adapter checkout validation."""

    try:
        return validate_checkout(lock, dependency_root)
    ...
    *, inventory_path: Path, lock_path: Path, dependency_root: Path
    ...
    try:
        adapter = FiveGAdapter.from_lock(
            ...
            state_root=Path(".synthran/fiveg"),
            timeout_seconds=30,
        )
        ...
        cores = capabilities.get("cores", [])
        rans = capabilities.get("rans", [])
        checks.append(
            DoctorCheck(
                "fiveg-ansible",
                True,
                f"machine API ready ({len(cores)} cores, {len(rans)} RANs)",
            )
        )
    return DoctorReport(tuple(checks))
